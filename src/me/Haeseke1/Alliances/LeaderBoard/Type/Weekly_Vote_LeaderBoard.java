...
public class Weekly_Vote_LeaderBoard extends LeaderBoard{
	
	public static Weekly_Vote_LeaderBoard weekly_Vote_LeaderBoard;
	
	public Weekly_Vote_LeaderBoard(List<Place> places) {
		super(places);
		weekly_Vote_LeaderBoard = this;
	}
	
	public void addPlace(int location, Head_Board hb){
		places.add(new Place(ChatColor.RED + "-Weekly Vote-", null, ChatColor.AQUA + "0", hb, location));
	}
	...
	public void update(){
		...
		Collections.sort(VotePlayer.voteplayers, new Comparator<VotePlayer>() {
			...
			public int compare(VotePlayer o1, VotePlayer o2) {
				return o1.weekly_votes - o2.weekly_votes;
			}
		...
		for(Place place : this.places){
			if(VotePlayer.voteplayers.size() >= place.location){
				...
				place.setScore(ChatColor.AQUA + "" + VotePlayer.voteplayers.get(place.location - 1).weekly_votes);
